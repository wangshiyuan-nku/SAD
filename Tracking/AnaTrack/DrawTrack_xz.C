...
//  TString filename = "rootfiles/trackCoul.root";
  ...
  void transco(int n,double* z, double* x);//transfer coordinator
...
//    cout<<"nstep:"<<nstep<<endl;
    for(int j=0;j<nstep;j++){
//      cout<<"z:"<<z[j]<<"; x:"<<x[j]<<endl;
    }

    if(iftransco) transco(nstep,z,x);
    TGraph* gr1 = new TGraph(nstop,z,x);
    gr.push_back(gr1);
    nDraw++;
  } 
  ...
  for(int m=0;m<1000;m++){
    ...
    if(iftransco&&fabs(Apz[m])>2.019){
      Apx1[m]=Apx*cost+Apz[m]*sint;
      Apx2[m]=-Apx*cost+Apz[m]*sint;
    } else{
      Apx1[m]=Apx;
      Apx2[m]=-Apx;
    }
...
void transco(int n,double* z, double* x){
  for(int i=0;i<n;i++){
    x[i]=x[i]*cost+z[i]*sint;
    z[i]=z[i]*cost-x[i]*sint;
  }
}
